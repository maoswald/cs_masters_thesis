...
On the other side there are still major issues due to the security concerns of classic \apufs as described in Sec. \ref{sec:attacksonarbiter} \cite{Ganji2016PACPUFs, Ruhrmair2014PUFOverview}.
...
The challenge bit length must match the number of stages in the \apuf that every stage gets a challenge bit assigned. 
...
If the attacker does not know the response of a single \apuf he can not separately attack every single \apuf of the \xpuf.
This protects the underling \apufs against direct attacks.
It is likely that the attacker has to model the whole \xpuf to be successful.

The number of combined \apufs $\gls{k}$ has the following two impacts on the \puf.
Using more \apufs increases the resistance against modeling attacks but also decreases the robustness which makes the \puf less reliable \cite{Becker2015ThePUFs, Ganji2015WhyPUFs}.
Robustness, also called stability, here means the likelihood that the \apuf responses equally to an \apuf with the same internal values but without noise when processing the same challenge under the same environmental conditions.
% This opposed relation between robustness and number of \apufs is due the fact that the errors are amplified by the \ac{XOR} function which leads to a maximum in the number of used \apufs for real applications \cite{Rostami2014RobustMatching}.
The decrease of the robustness of the entire \xpuf caused by using more \apufs is due the fact that the less stable challenges of every \apuf are accumulated by the \ac{XOR} function.
...
Using $12$ \apufs in a \xpuf leads to a total robustness of approximately $50 \%$ and is not usable as the response is not stable reproducible as Ganji et al. implied \cite{Ganji2015WhyPUFs}.
However Yu et al. suggested that Ganji et al. is not precise enough and the robustness of a \xpuf with $\gls{k} = 12$ is $67 \%$ for an even lower robustness of $95.7 \%$ for the included \apufs \cite{Yu2016AAuthentication}.
Considering that the limit for size $\gls{k}$ of a \xpuf depends on the required stability.
...
When applying the same challenge to all \apufs the same bit of the challenge will affect the response of every \apuf with the same likelihood. 
...
The theoretical model represents an \apuf and serves thus as representation used as result of attacks explained in Sec. \ref{cap:mla}.
...
\ac{CPLD} follow a similar principle as the \ac{FPGA} but with some differences for example being smaller in terms of resources or less complex structure.
...
It can lead to bias in the mean of the distribution of the delay differences of a \apuf, explained in Sec. \ref{sec:arbiter}, making the responses predictable and introduces a lack of randomness \cite{Majzoobi2010FPGALines, Morozov2010AnFPGA}.
The variation of the traces can even be higher than the manufacturing variations of the stage implementations so that the response is mainly decided by the routing delays instead of the different variation combination of the used stage paths \cite{Majzoobi2009TechniquesPUFs}.
...
For the hardware realization of a single stage two 2-to-1 \ac{MUX} are used as suggested by Lee et al. \cite{Lee2004AApplications} or Rührmair et al. \cite{Ruhrmair2013PUFData}.
...
Additional differences like clipping parts of an continuous signal when exceeding a specific voltage does not apply here as the \apuf
...
Ensured by the cross-coupling only one competing transition can win and the symmetric layout prevents from circuit design based bias.
...
This work does not study physical \apuf implementations further as they are not part of the attacks.
...
To evaluate the response $\gls{r}$ of a challenge $\gls{c}$ of a specific \puf the sum of $\Delta D_{\mathrm{Model}}(c)$ and the noise values $\Delta D_{\mathrm{Noise Stage}}$ and $\Delta D_{\mathrm{Noise Arbiter}}$ is computed.
...
The current stage and every following stage could swop the signals and change the final response.
\todo{proofbyexample?}
...
Noise that occurs at the arbiter $\Delta D_{\mathrm{Noise Arbiter}}$ can be modeled as a randomly drawn value of a normal distribution with zero mean and given variance $\sigma_{Arbiter Noise}^2$ depending only on the used electronic component.
...
However the noise free model $\Delta D_{\mathrm{Model}}(c)$ is the part with the hidden information which want to be exposed.
...
Here introduced models are used in both to simulate attackable \apuf instances in Sec. \ref{sec:pufsimulation} and to attack them in Sec. \todo{ref}.
...
For a good result the responses of the \acp{CRP} should have a high stability as possible.
...
In this thesis I assume to have free access to the \puf.
...
Another technique to create accurate \apuf models are \ac{ANN} and \ac{PAC} learning, although \acp{ANN} need a relative large amount of \acp{CRP} to achieve a high prediction accuracy of new challenges \cite{Hospodar2012MachineUsability}.
...
With these values the attacker can create a exact model of the \apuf as explained in Sec. \ref{sec:theoretical}.
Different combinations of machine learning attacks and side channel attacks has been proposed as well \cite{Mahmoud2013CombinedPUFs, Xu2014Hybrid}.
