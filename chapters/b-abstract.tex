...
% old:
% Physical unclonable functions (\acsp{PUF}) ought to fulfill this need with the possibility to derive secret keys from their intrinsic hardware imperfections that can be used but not be read out easily.
...
Physical unclonable functions (\acsp{PUF}) ought to fulfill this need with the possibility to derive secret keys from their intrinsic hardware imperfections
These imperfections are linked inseparable with the device and can be easily used to derive secret keys from but are hard to read out.
In addition, there is a high chance that they are unique to every instance.
If that is true, they can be used to securely identify devices.
...
Machine learning attacks in combination with a feasible amount of \aclp{CRP} make it possible to create models of  Arbiter \acsp{PUF} and \acs{XOR} Arbiter \acsp{PUF}, which can be used to predict responses for unknown challenges.
...
To counteract these instabilities, the principle of \acl{MV} is applied to Arbiter \acsp{PUF}, which are used in \acs{XOR} Arbiter \acsp{PUF}.
...
Its impacts on the stability of Arbiter \acsp{PUF} and \acs{XOR} Arbiter \acsp{PUF}, and relevant attacks are verified by simulations.
...
Thus, large Majority \acs{XOR} Arbiter \acsp{PUF} constitute a foundation for machine learning attack resistant \acsp{PUF}.
Since they are vulnerable to invasive attacks, they do not protect secret keys completely.
